...
export async function signIn(
  ...
  // iOS Safari/WKWebView blocks popups. Drive OAuth in this window instead.
  if (preview && ios) {
    const next = encodeURIComponent(callbackURL.startsWith("/") ? callbackURL : "/search");
    window.location.assign(
      `/auth/popup?providerId=${encodeURIComponent(providerId)}&next=${next}`,
    );
    return;
  }
  ...
  else throw new Error("Could not start Google/X sign-in.");
}
