...
export async function handleAuthPopupRequest(request: Request): Promise<Response> {
  ...
  if (done) {
    ...
    return new Response(completionHtml(message, next), {
      status: 200,
      headers: {
  ...
  const next = safeNext(url.searchParams.get("next"));
  const back = `${url.origin}/auth/popup?done=1&next=${encodeURIComponent(next)}`;
  try {
...
function completionResponse(message: PopupMessage, next = "/search"): Response {
  return new Response(completionHtml(message, next), {
    status: 200,
    headers: {
      "content-type": "text/html; charset=utf-8",
...
function completionHtml(message: PopupMessage, next = "/search"): string {
